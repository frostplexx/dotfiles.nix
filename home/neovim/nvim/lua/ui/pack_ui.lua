...
local function get_plugin_status(plugin, deleted_plugins)
    if deleted_plugins[plugin.name] then return "deleted" end
    if not plugin.is_configured then return "orphaned" end
    if not plugin.is_loaded then return "unloaded" end
    return "loaded"
end
...
local function is_pack_plugin(plugin_name)
    return vim.tbl_contains(vim.tbl_map(function(p) return p.spec.name end, vim.pack.get()), plugin_name)
end
    ...
    for _, map in ipairs(keymaps) do
        ...
        if type(keys) == "table" then
            for _, key in ipairs(keys) do
                vim.api.nvim_buf_set_keymap(buf, mode, key, type(action) == "string" and action or "",
                    vim.tbl_extend("force", opts or {}, type(action) == "function" and { callback = action } or {}))
            end
        else
            vim.api.nvim_buf_set_keymap(buf, mode, keys, type(action) == "string" and action or "",
                vim.tbl_extend("force", opts or {}, type(action) == "function" and { callback = action } or {}))
        end
    end
